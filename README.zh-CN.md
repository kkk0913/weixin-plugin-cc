...
- **扫码登录** — 扫码登录，会话自动保存和恢复
- **长轮询消息投递** — 实时微信消息桥接
- **按账号+会话持久化 context_token** — 重启后自动恢复最近一次可用的会话 token
- **媒体支持** — 收发图片、文件（最大 50MB），`mp3`、`mp4`、`wav`、`m4a` 等媒体文件默认作为普通附件发送
- **访问控制** — 配对模式（默认）、白名单、禁用
...
- 当前支持的出站媒体：`image_item`、`file_item`
- `mp3`、`mp4`、`wav`、`m4a` 等媒体文件默认通过 `file_item` 发送
- `voice_item` 暂不支持
- `video_item` 暂不支持

### 运行时说明

- `context_token` 会按微信账号 + 会话维度持久化，并在出站回复时优先复用
- 长轮询会保证同一聊天内消息顺序，同时允许不同聊天并行处理
